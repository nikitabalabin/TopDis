...
class BetaVAE_HL(nn.Module):
    """Model proposed in original beta-VAE paper(Higgins et al, ICLR, 2017)."""

    def __init__(self, z_dim=10, nc=3):
        super(BetaVAE_HL, self).__init__()
        self.z_dim = z_dim
        self.nc = nc
        self.encoder = nn.Sequential(
            nn.Conv2d(nc, 32, 4, 2, 1),          # B,  32, 32, 32
            ...
            nn.ReLU(True),
            nn.Conv2d(256, 512, 4, 1),            # B, 512,  1,  1
            nn.ReLU(True),
            View((-1, 512*1*1)),                 # B, 512
            nn.Linear(512, z_dim*2),             # B, z_dim*2
        )
        self.decoder = nn.Sequential(
            nn.Linear(z_dim, 512),               # B, 256
            View((-1, 512, 1, 1)),               # B, 256,  1,  1
            nn.ReLU(True),
            nn.ConvTranspose2d(512, 256, 4),      # B,  64,  4,  4
            nn.ReLU(True),
            nn.ConvTranspose2d(256, 128, 4, 2, 1), # B,  64,  8,  8
            nn.ReLU(True),
