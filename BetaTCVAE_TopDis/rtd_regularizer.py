...
#             _, _, _, z = model(x_batch)
            #_, z, _ = model(x_batch, current_kl_step) #dava
    
            _, mu, logvar = model(x_batch) #controlvae
            z = model.reparametrize(mu, logvar) #controlvae
    
        else:
#             z = model.encode(x_batch)[:, :model.z_dim, 0, 0]
            #z, _ = model.encoder(x_batch) #dava
            #z = model._encode(x_batch)[:, :model.z_dim, 0, 0] #controlvae
            z = model.encoder.encode(x_batch)[1][:,:,0] #beta-tcvae
        if self.weightnorm_sampler:
#             importance = model.decode[0].weight.norm(dim=0).flatten().detach().clone().cpu()
            importance = model.decoder.decode[0].weight.norm(dim=0).flatten().detach().clone().cpu() # dava #beta-tcvae
            #importance = model.decoder[0].weight.norm(dim=0).flatten().detach().clone().cpu() # controlvae
        ...
        if sgn > 0:
            ...
            mask = (prob - C > 0)
            C = -C
        z_valid = z[mask][:,:,None,None].clone()
        z_new = z_valid.clone()
        z_new[:, i] = normal_sinv(prob[mask] + C)[:,None,None] * s_batch + m_batch
        z_reg = torch.cat([z_valid, z_new])
#         x_reg = model.decode(z_reg).sigmoid()
        #z_reg = z_reg.flatten(1,-1) # controlvae
        #x_reg = model._decode(z_reg).sigmoid() # controlvae
        x_reg = model.decoder.decode(z_reg).sigmoid() # beta-tcvae
