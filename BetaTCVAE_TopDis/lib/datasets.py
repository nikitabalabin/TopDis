...
class MPI3D(object):
    ...
    def __init__(self):
        loc = 'data/real3d_complicated_shapes_ordered.npz'
        self.dataset_zip = np.load(loc, encoding='latin1')
        self.imgs = torch.from_numpy(self.dataset_zip['images']).float().div(255).permute(0,3,1,2)

    def __len__(self):
        return self.imgs.shape[0]

    def __getitem__(self, index):
        x = self.imgs[index].view(3, 64, 64)
        return x
